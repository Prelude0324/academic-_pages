...
Fig. 3 shows the Richards-Baker Index (a flashiness index employed to evaluate the sub-daily hydrological alteration caused by hydropeaking) in the downstream river of Longtan Reservoir (4,900 MW), located in the Pearl River Basin in GX Province.
$$
RBIndex= \sum\nolimits_{t = 1}^T {0.5(\left| {{q_{t + 1}} - {q_t}} \right| + \left| {{q_t} - {q_{t - 1}}} \right|} )/\sum\nolimits_{t = 1}^T {{q_t}}
$$
